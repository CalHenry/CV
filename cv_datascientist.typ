...
// Custom imports:
// Comment the import you don't want to use the real info or anonymised info at compile
//#import "modules/personal-info.typ": real as personal-info
#import "modules/personal-info.typ": anom as personal-info
// List of all the entries of 'personal-info' that are used in the doc:
// - name, name_lower (recreate linkedin url)
// - email
// - phone_fr

#import "modules/custom_content.typ": cloud_skills, data_viz_extra, python_libs, ml_models
// custom_content.typ" is used to customize add content to the doc without changing the overall code. + easy to see where content is added
  ...
  *Traitement dans un environnement BigData Cloud* _(AWS, Databicks)_: \
  ...
  [
    *Chargé d'étude et d'évaluation* (_Ministère du Travail et de la Santé_) #box(baseline: 20%, [#sicon(slug: "r", size: 1.2em, icon-color: "default") ])
    ...
    - Création d'un modèle de prévision de l'âge de départ à la retraite
  ],
    [_09/2023 - 08/2024_]
)
#v(-1.4em)
...
#section(text(size: 14pt, [Projets personnels]))
...
#entry(
  [
    *#link("https://github.com/CalHenry/Wildchat1M-clustering", `Wildchat-1M clustering`)* -
  ...
  - Librairies : #link("https://www.langchain.com", `langchain`), #link("https://docs.crawl4ai.com", `crawl4ai`), #link("https://docs.pola.rs", `polars`)
  ],
  [_08/2025_]
)
  ...
  _Assistant IA pour git commits_ #python\
    ...
    - Librairies : #link("https://ollama.com", `ollama`) pour le LLM en local, #link("https://textual.textualize.io", `textual`) et #link("https://typer.tiangolo.com", `typer`) pour l'application CLI.
  ],
  [_09/2025_],
)
...
#table(
  ...
  text("Programmation & Data", weight: 600),
  [Python #python_libs, R, SQL, VBA],
  text("Statistiques", weight: 600),
  [Machine learning, Économétrie, Deep learning],
  text("Machine learning - IA", weight: 600),
  [Scikit-learn, PyTorch, Tensorflow, Langchain, MLFlow] + ", " + ml_models,
  ..cloud_skills,
  text("Dashboards", weight: 600),
  [Shiny, Streamlit, Marimo] + ", " + data_viz_extra,
